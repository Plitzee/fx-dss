...
from contig import run_length, report

# so phien LIEN MACH toi thieu moi mo hinh can, tinh den phien t
NEED={"RW-cc":2,"RW-GK":2,"MA20-cc":20,"MA20-GK":20,"EWMA94":20,"GARCH-t":20,
      "HAR-Park":22,"HAR-GK":22,"HAR-RS":22,"HAR-YZ":27,"HAR-RV5":22}
...
def fc_har(s,t,run=None,need=22):
    lv=np.log(s[:t+1]); X=har_design(lv); y=lv[1:]; Xd=X[:-1]
    ok=np.isfinite(Xd).all(1)&np.isfinite(y)
    if run is not None:                      # bo hang thiet ke bac qua lo hong
        ok&= (run[:t+1][:-1]>=need)
...
ALL={}
print("="*92); print("CHAY 6 CAP TIEN — CO RAO CHAN LIEN MACH"); print("="*92)
for PAIR in PAIRS:
    ...
    RUN=run_length(m.Date.values); GAPOK=np.ones(n,bool)
    gd=pd.Series(m.Date).diff().dt.days.values
    GAPOK[1:]=gd[1:]<=4
    report(m.Date.values,label=PAIR)
    ...
    for t in range(MIN_TRAIN,n-1):
        if not GAPOK[t+1]:          # chinh buoc t -> t+1 da bac qua lo hong
            continue
        R=RUN[t]
        def ok_(k): return R>=NEED[k]
        if ok_("RW-cc"):   F["RW-cc"][t+1]=max(px["cc"][t],EPS)
        if ok_("RW-GK"):   F["RW-GK"][t+1]=max(px["gk"][t],EPS)
        if ok_("MA20-cc"): F["MA20-cc"][t+1]=px["cc"][t-19:t+1].mean()
        if ok_("MA20-GK"): F["MA20-GK"][t+1]=px["gk"][t-19:t+1].mean()
        if ok_("EWMA94"):  F["EWMA94"][t+1]=ew[t]
        if ok_("HAR-Park"):F["HAR-Park"][t+1]=fc_har(px["park"],t,RUN,22)
        if ok_("HAR-GK"):  F["HAR-GK"][t+1]=fc_har(px["gk"],t,RUN,22)
        if ok_("HAR-RS"):  F["HAR-RS"][t+1]=fc_har(px["rs"],t,RUN,22)
        if ok_("HAR-YZ"):  F["HAR-YZ"][t+1]=fc_har(px["yz5"],t,RUN,27)
        if ok_("HAR-RV5"): F["HAR-RV5"][t+1]=fc_har(px["rv5"],t,RUN,22)
        if t-last>=50:
            try:
        ...
        if gp is not None:
            ...
            if R>=NEED["GARCH-t"]: F["GARCH-t"][t+1]=gs/1e4
    RES={}; TG={}
