...
| 58 | 14/09/2026 | **Trọng số ngày trong tổ hợp sản xuất** (`kiem_trongso.py`) — bước thứ ba nhắm vào khoảng cách QLIKE ~7,78% mà `BCL_TIEUCHI.md` phát hiện, sau khi tách nhảy bằng ngưỡng (dòng 49) và bằng bipower (dòng 57) đều thất bại. Giả thuyết H15, 3 cấu hình (thêm mô hình HAR cổ điển Corsi 2009 làm thành viên thứ tư và/hoặc đổi trung bình đều sang trọng số Hedge — cùng cơ chế đã đo tốt ở tầng xác suất, `balop.ToHopTrucTuyen`), tiêu chí phủ định CHỐT TRƯỚC ở `docs/TOHOP_TRONGSO_TIEUCHI.md`. Tái tạo đúng cơ chế `du_bao_san_xuat()` như dòng 57 | **3** | **ÂM — và đây là lần THỨ BA liên tiếp một giả thuyết nhắm đúng khoảng cách 7,78% thất bại.** Tốt nhất (B0 Hedge) chỉ **−0,25%** trên kiểm tra (p=0,551, 4/6 cặp) — không tiệm cận mức 7,78% đã chẩn đoán, và không đạt cả hai điều kiện phủ định. **Hệ quả quan trọng hơn cả kết quả âm**: vì tách nhảy (ngưỡng lẫn bipower) và trọng số tổ hợp (thành viên lẫn cách tổ hợp) đều không tái tạo được cải thiện đó khi kiểm trong đúng cơ chế `du_bao_san_xuat()` thật, khả năng cao nhất là con số 7,78% phản ánh đặc thù của khung hồi quy một phương trình dùng để chẩn đoán nó (trên đầu ra đã hiệu chỉnh log-chuẩn), không phải một khiếm khuyết sửa được trong cách tổ hợp ba mô hình con. **Đề nghị rút lại phát biểu "tầng 2 để lại ~8-10% QLIKE trên bàn"** cho tới khi có bằng chứng trực tiếp hơn (kiểm phép hiệu chỉnh log-chuẩn — việc còn treo). Xem `docs/TOHOP_TRONGSO_TIEUCHI.md` Phụ lục A |
| — | 14/09/2026 | **Chẩn đoán đóng nốt việc còn treo ở dòng 58** — dựng lại `log_rv(t)` đúng, không rò rỉ (kiểm tường minh: mỗi hàng một ngày liên tục, không trùng `(pair,ngày)`, không khoảng trống; lấy đúng `y_bien_do` của hàng liền trước cùng cặp) | **0** (chẩn đoán, không phải cấu hình mới) | **Cho ra −63,30%, không phải −7,78% đã ghi ở `BCL_TIEUCHI.md` Phụ lục A** — khác nhau một bậc độ lớn dù cùng một ý tưởng. R² huấn luyện chỉ tăng khiêm tốn (0,535→0,585) trong khi QLIKE trung vị giảm hơn một nửa — lệch nhau kiểu này là dấu hiệu số hạng hiệu chỉnh log-chuẩn toàn cục `exp(0,5·s²)` (tính một lần trên huấn luyện, áp đều cho kiểm tra) đang khuếch đại một thay đổi nhỏ ở s² thành thay đổi lớn ở QLIKE, không phải bằng chứng cải thiện dự báo thật cỡ đó. **Kết luận: chính khung đo dùng để chẩn đoán "khoảng cách 7,78%" không ổn định — không nên trích dẫn con số đó, kể cả định tính, trong luận văn.** Không ảnh hưởng tới các phán quyết đã chấm trực tiếp qua `du_bao_san_xuat()` (dòng 47, 49, 57, 58) — chỉ riêng diễn giải "tầng 2 để lại 8-10% QLIKE" cần rút lại. Xem `docs/TOHOP_TRONGSO_TIEUCHI.md` Phụ lục B |

| 59 | 15/09/2026 | **Vá đuôi — HƯỚNG THỨ TÁM: ĐUÔI TRỰC TUYẾN (ACI/AgACI)** (`va_duoi_aci.py`) — Gibbs & Candès (2021) *NeurIPS* 34:1660–1672; Zaffran et al. (2022) *ICML* 162:25834–25866. Giả thuyết H17a/H17b, 5 cấu hình, giao thức và tiêu chí phủ định CHỐT TRƯỚC ở `docs/DUOI_ACI_TIEUCHI.md` (commit 8ab4eb5). **Khác bảy hướng trước ở LỚP vấn đề, không ở ước lượng đuôi**: bảy hướng đều đề xuất k cấu hình rồi chọn 1 trên kiểm định — mà `TONG_QUAN` mục 5.1 đã chẩn đoán chính bước đó là chỗ hỏng, và `DUOI_EVT` A4(h) đo được hệ quả (lần thứ tư chọn-trên-kiểm-định chọn sai). Hướng này **bỏ bước đó**: A1 chốt trong văn bản tiêu chí trước khi thấy số. Cơ chế đã có sẵn trong repo nhưng đặt sai tầng — `conformal.py::chay_aci` chạy sản xuất cho tập dự báo 90% (dòng 24) và chưa từng được áp cho tầng VaR/ES | **5** | **KIỂM ĐỊNH DƯƠNG, đoạn kiểm tra CHƯA MỞ.** (a) A1 sạch **12/12 ô ở CẢ Kupiec LẪN DQ**; mốc V0: Kupiec bác bỏ 2/12, DQ bác bỏ 3/12. **H17a ĐẠT** (6/6 cặp cả hai mức). (b) Hai ô V0 đang hỏng đều được vá **kể cả DQ**: USDCHF α=1% vi phạm 2,19%→1,28%, Kupiec 0,015→0,529, **DQ 0,003→0,879**, tỷ lệ ES 1,329→0,954; USDCAD α=1% DQ 0,049→0,984. Đây là **lần đầu trong dự án một hướng vá đuôi làm DQ không bác bỏ** ở ô đang hỏng — bảy hướng trước, kể cả EVT (gần nhất), đều vá được tần suất mà không chạm được DQ (`DUOI_EVT` A4(d)). (c) **Cảnh báo chốt trước KHÔNG kích hoạt**: `ac1` của USDCHF đi từ **+0,063** (vi phạm dồn cụm — đúng khiếm khuyết đang có) về **−0,013**, không phải tự tương quan âm nhân tạo; theo mục 3 nếu DQ đạt nhờ chiều âm thì phải ghi là thất bại của H17b. (d) **Tự kiểm 2 là bằng chứng cơ chế độc lập với dữ liệu FX**: dưới dịch chuyển phân phối biết trước, ACI giữ 0,0503/0,0118 so đích 0,05/0,01 trong khi phân vị **tĩnh** trượt về 0,1406/0,0990 (thiếu phủ ~3× và ~10×). (e) **Khai báo sửa công thức cận TRƯỚC mọi số liệu thật**: mục 8 ban đầu viết cận một phía `(α₁+γ)/(T·γ)`; cận hai phía đúng dùng `max{α₁,1−α₁}`. Lỗi do chính tự kiểm 3 bắt trên dữ liệu mô phỏng — kết luận định tính không đổi và còn mạnh hơn (cận 0,176–0,183, **lỏng hơn chính α ở cả hai mức**). Ghi kèm một giả thiết bị vi phạm: phép dẫn giả định `α_t` không bị chặn, còn cài đặt này và cả `chay_aci` sản xuất đều chặn — nên mọi dòng "trong cận" đọc kèm tỷ lệ `α_t` chạm sàn. (f) **Phụ lục A4 nói thẳng vì sao CHƯA phải bằng chứng quyết định**: đoạn kiểm định chính là nơi bảy hướng trước cũng trông ổn; cái mới là A1 được chỉ định trước, cơ chế kiểm độc lập, và cận vẫn lỏng nên **không được viết "định lý bảo đảm nên sẽ đạt trên kiểm tra"**. Xem `docs/DUOI_ACI_TIEUCHI.md` Phụ lục A |
| — | 15/09/2026 | **Vá rò rỉ niêm phong ở `va_duoi.py`/`va_duoi_evt.py`** — việc tồn đã khai ở `RUIRO_ML.md` A4, nay đo được và vá (commit ea96cee). Cả hai dùng `api.main.noi_chuoi`, nối dữ liệu live tới 2026-09, mà mục 2 dành riêng 2026-01→08 cho niêm phong; vì `TEST_TU`=2023-11-20, toàn bộ phần 2026 rơi vào nhãn `g==2` | **0** (bản vá, không phải cấu hình mới) | Đo 15/09/2026: **179–180/729 phiên mỗi cặp, tức 24,6% đoạn kiểm tra**, là dữ liệu niêm phong. Sửa ở `nap()` nên vá luôn `va_duoi_evt.py` (import chung). Chạy lại `va_duoi.py`: `output/va_duoi.json` **không đổi** — xác nhận bản vá chỉ chạm đoạn kiểm tra, không động số liệu chọn-trên-kiểm-định đã báo cáo. `shadow_tohop.py` cũng dùng `noi_chuoi` nhưng **không** phải rò rỉ: nó sinh dự báo thật cho sổ dự báo, không chấm điểm trên đoạn nào. **Hệ quả cho dòng 48 — xem đính chính ngay dưới.** |

> **ĐÍNH CHÍNH dòng 48 (EVT/POT) — 15/09/2026.** Số liệu **đoạn kiểm tra** của
> dòng 48 và của `DUOI_EVT.md` Phụ lục A được chấm **trước** bản vá ở dòng "—"
> trên, nên đoạn kiểm tra khi đó còn lẫn **24,6%** dữ liệu niêm phong (2026-01→09).
> Mọi con số kiểm tra của EVT — gồm "V0 0,4913 → E3 0,2749 (giảm 44%)",
> "USDJPY α=1% Kupiec 0,005 → 0,198", "USDCHF tỷ lệ ES 1,131 → 1,008" và phán
> quyết ĐK1/ĐK2/ĐK3 — phải đọc kèm ghi chú này.
>
> **Quyết định 15/09/2026: KHÔNG chạy lại.** Chạy lại sẽ là **lần mở thứ hai**;
> được phép theo quy tắc 3 (lỗi code, không phải kết quả xấu) nhưng phán quyết
> của dòng 48 vốn đã là **ÂM** (trượt ĐK2 và ĐK3) và EVT **không đổi sản xuất**,
> nên số liệu sạch hơn không đổi kết luận nào. Ghi chú thay vì tiêu một lần mở.
> Số liệu **kiểm định** của dòng 48 **không bị ảnh hưởng** — đoạn kiểm định kết
> thúc 2023-11-20, trước mọi dữ liệu niêm phong.

### Tổng kết cho phần hiệu chỉnh bội của luận văn

| khoản | số lượng |
|---|---|
| Cấu hình **mô hình** đã thử trên tập phát triển | **~1.345** (1.322 đến dòng 51 + 6 dòng 53 lớp phủ DML + 6 dòng 55 CNN nến + 3 dòng 57 HAR-J + 3 dòng 58 trọng số ngày + **5 dòng 59 đuôi trực tuyến ACI**) |
...
~1.345 cấu hình mô hình là lý do mọi kết luận về **mô hình** đều chỉ được phát
...
| 3 | Mọi cấu hình/giả thuyết đã thử trên tập phát triển được đếm vào mục 5 | ✅ Đã cập nhật đến dòng 59 | Bảng tổng kết ở trên, ~1.345 cấu hình + 9.258 giả thuyết pattern + 594 giả thuyết nhân quả |
| 7 | Hướng thứ tám (đuôi trực tuyến ACI, dòng 59) đã quyết mở hay không mở đoạn kiểm tra | ⚠️ **Chưa — đang chờ chữ ký mục 9 của `DUOI_ACI_TIEUCHI.md`** | Kiểm định DƯƠNG (12/12 Kupiec và DQ, vá được cả USDCHF lẫn USDCAD kể cả DQ). Đoạn kiểm tra chưa mở. Mục 9 của văn bản chốt trước đòi chữ ký **trước** khi số liệu được chấm — quyết định 15/09/2026 là ký trước, chưa chạy `--mo-kiem-tra` |
