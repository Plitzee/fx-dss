...
A **sealed evaluation set** — six cross pairs, NZD/USD, and 2026 data as available at the pre-specified cutoff date — is reserved and is opened exactly once, after the final configuration has been frozen in writing; because the sealed set includes calendar year 2026 while the study concludes before that year ends, "2026" refers to whatever portion of the year has been collected through the continuously operating daily pipeline by the cutoff date, not the completed calendar year. No retuning is permitted after the sealed set is opened.
